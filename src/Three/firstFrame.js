...
import {setupRenderTargets} from "./render-targets";
...
export function drawFirstFrame(mount, type = InitialTextureTypes.CIRCLE) {
  // writes the bufferTargets to an array assigned to the window
  setupRenderTargets();

  // Grab the invisible canvas context that we can draw initial image data into
  window.buffer.bufferCanvas = document.querySelector('#buffer-canvas');
